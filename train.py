...
def run(config):
    ...
    # Update the config dict as necessary This is for convenience, to add settings derived from the user-specified
    # configuration into the config-dict (e.g. inferring the number of classes and size of the images from the
    # dataset, passing in a pytorch object for the activation specified as a string)
    ...
    if config["dataset"] == "FFHQ":
        ...
        print("rooooot:", root)
        dataset = FFHQ(root=root, transform=transform, batch_size=batch_size * config["num_D_accumulations"],
                       imsize=config["resolution"])
        ...
        batch_size = config['batch_size']
        dataset = Celeba(root=root, transform=transform, batch_size=batch_size * config["num_D_accumulations"],
                         imsize=config["resolution"])
        data_loader = DataLoader(dataset, batch_size, shuffle=True, drop_last=True)
        loaders = [data_loader]
        ...
        dataset = CocoAnimals(root=root, batch_size=batch_size * config["num_D_accumulations"], classes=classes,
                              transform=transform, imsize=config["resolution"])
        data_loader = DataLoader(dataset, batch_size * config["num_D_accumulations"], drop_last=True,
        ...
        for i, batch_data in enumerate(pbar):
            ...
            if config["unet_mixup"]:
                ...
                target_map = torch.cat(
                    [CutMix(config["resolution"]).cuda().view(1, 1, config["resolution"], config["resolution"]) for _ in
                     range(n_mixed)], dim=0)
            ...
            if config["slow_mixup"] and config["full_batch_mixup"]:
                ...
                r_mixup = 0.5 * min(1.0, state_dict[
                    "epoch"] / warmup_epochs)  # r is at most 50%, after reaching warmup_epochs
            ...
            go_ahead_and_sample = (not (state_dict['itr'] % config['sample_every'])) or (
            ...
            if go_ahead_and_sample:
                ...
                if config['G_eval_mode']:
                    ...
                    train_fns.save_and_sample(G, D, G_ema, z_, y_, fixed_z, fixed_y,
                                              state_dict, config, experiment_name, sample_only=True, use_real=True,
                        ...
                        with torch.no_grad():
                            ...
                            if config["dataset"] == "coco_animals":
                                ...
                                train_fns.save_and_sample(G, D, G_ema, z_[:n], y_[:n], fixed_z[:n], fixed_y[:n],
                                                          state_dict, config, experiment_name + "_mix",
                                                          sample_only=True, use_real=True, real_batch=mixed, mixed=True,
                                                          target_map=target_map[:n])

                            else:
                                real_batch = dataset.fixed_batch()
                                fixed_Gz = nn.parallel.data_parallel(which_G, (
                                    fixed_z[:n], which_G.shared(fixed_z[:n])))  #####shouldnt that be fixed_y?
            ...
            if not (state_dict['itr'] % config['test_every']):
                ...
                is_mean, is_std, fid = train_fns.test(G, D, G_ema, z_, y_, state_dict, config, sample,
                                                      get_inception_metrics, experiment_name, test_log, moments="train")
                ###
            ...
            if (i + 1) % loss_steps == 0:
                with open(os.path.join(config["base_root"],
                                       "logs/inception_metrics_" + config["random_number_string"] + ".p"), "wb") as h:
                    pickle.dump(inception_metrics_dict, h)
                    print("saved FID and IS at", os.path.join(config["base_root"], "logs/inception_metrics_" + config[
                        "random_number_string"] + ".p"))
